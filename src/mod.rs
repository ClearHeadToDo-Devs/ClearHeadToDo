...
pub fn update_action_vertex_completion_status(
    datastore: MemoryDatastore,
    action_id: Uuid,
    new_status: bool,
) -> Result<MemoryDatastore, Box<dyn Error>> {
    let update_result = datastore.set_vertex_properties(
        create_property_query_for_vertex(action_id, "Completed").into(),
        new_status.into(),
    );

    match update_result {
        Ok(()) => Ok(datastore),
        Err(_) => Err("Failed to update action completion status".into()),
    }
}
...
pub fn get_action_by_id(datastore: MemoryDatastore, action_id: Uuid) -> Action {
    ...
    ActionBuilder::default()
        ...
        .set_priority(extracted_action[0].props[2].value.as_u64().unwrap().into())
        .build()
}
    ...
    mod db_ops {
        ...
        fn update_action_completion_status() {
            ...
            let action_property_query =
                create_property_query_for_vertex(action_id.clone(), "Completed");

            let update_result =
                update_action_vertex_completion_status(datastore, action_id.clone(), true).unwrap();
        ...
        fn update_action_priority() {
            ...
            let property_query = create_property_query_for_vertex(action_id.clone(), "Priority");

            let update_result =
                update_action_vertex_priority(datastore, action_id, Priority::Low).unwrap();

            assert!(
                update_result.get_vertex_properties(property_query).unwrap()[0].value
                    == Number::from(4)
            )
        }
        ...
        fn add_default_action_to_datastore() {
            ...
            assert!(addition_result.0.get_vertex_count().unwrap() == 1)
        }

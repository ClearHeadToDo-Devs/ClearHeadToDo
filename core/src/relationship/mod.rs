...
impl RelationshipListManagement for Vector<Relationship> {
    ...
    fn select_relationship_by_id(&self, id: Uuid) -> Result<Relationship, String> {
        let query_result = self
            .iter()
            .find(|relationship| relationship.get_id() == id)
    ...
    fn remove_with_id(&self, id: Uuid) -> Result<Self::L, Box<dyn Error>> {
        ...
        let target_index = cloned_list
            .index_of(&cloned_list.select_relationship_by_id(id)?)
    ...
    fn change_relationship_variant(
        ...
        let updated_relationship = self
            .select_relationship_by_index(index)?
            .set_variant(variant)?;
...
impl RelationshipListViewer for Vector<Relationship> {
    ...
    fn id_is_present_in_participant_1_list(&self, id: Uuid) -> bool {
        let query_result = self
            .iter()
            .find(|relationship| relationship.get_participant_1() == id);
    ...
    fn id_is_present_in_participant_2_list(&self, id: Uuid) -> bool {
        let query_result = self
            .iter()
            .find(|relationship| relationship.get_participant_2() == id);
    ...
    fn id_is_present_in_either_participant_list(&self, id: Uuid) -> bool {
        self.id_is_present_in_participant_1_list(id) || self.id_is_present_in_participant_2_list(id)
    }
    fn filter_by_participants(
        ...
        match participant_list.as_str() {
            "participant_1" | "p1" | "1" | "list1" => Ok(self.get_participant_1_list_for_id(id)?),
            "participant_2" | "p2" | "2" | "list2" => Ok(self.get_participant_2_list_for_id(id)?),
    ...
    fn get_either_participant_list_for_id(&self, id: Uuid) -> Result<Self::L, Box<dyn Error>> {
        match self.id_is_present_in_either_participant_list(id) {
            true => {
                ...
                for relationship in self.iter() {
                    if relationship.get_participant_1() == id
                        || relationship.get_participant_2() == id
                    {
    ...
    fn get_children_for_id(&self, id: Uuid) -> Result<Vector<Uuid>, Box<dyn Error>> {
        ...
        let children_relationship_list = participant_1_list.iter().filter(|relationship| {
            relationship.get_variant() == RelationshipVariant::create_parental()
        });
